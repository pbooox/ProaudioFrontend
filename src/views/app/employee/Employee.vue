<template>
  <div>
    ...
    <b-modal id="modal-des" title="Desactivar">
      <p class="my-4">
        ¿Desea desactivar el empleado: {{ form.name }} ?
      </p>
    ...
    <b-modal id="modal-react" title="Activar">
      <p class="my-4">
        ¿Desea activar el empleado: {{ form.name }} ?
      </p>
      <template #modal-footer="{}">
        <b-button
          size="sm"
      ...
      <b-form>
        ...
        <b-form-group label="Correo:">
          <b-form-input
            ...
            :state="!$v.form.mail.$error"
            placeholder="Ingresar correo"
          ></b-form-input>
          <b-form-invalid-feedback
            >Debe ingresar el correo</b-form-invalid-feedback
          >
        </b-form-group>
        <b-form-group label="CUI:">
          ...
          ></b-form-input>
          <b-form-invalid-feedback
            >Debe ingresar el CUI</b-form-invalid-feedback
          >
        </b-form-group>
      </b-form>
      <template #modal-footer="{}">
    ...
    <b-row>
      <b-colxx xxs="12">
        ...
        <vuetable
          ...
          no-data-template="No se encontraron datos"
          :api-url="apiBase"
...
export default {
  ...
  methods: {
    ...
    onSave() {
      const me = this;
      this.$refs["modalSave"].hide();
      axios.post(apiUrl + "/employees/create", {
          form: this.form,
        })
        .then((response) => {
          me.$notify(
    ...
    onState() {
      ...
      if (this.form.state == 1) {
        axios
          .put(apiUrl + "/employees/deactivate", {
            id: this.form.id,
          })
          .then((response) => {
            me.$notify(
              "warning",
              "Desactivado",
              "Empleado " + this.form.name + " desactivado",
              { duration: 3000, permanent: false }
            );
            me.$refs.vuetable.refresh();
          })
        ...
        axios
          ...
          .then((response) => {
            me.$notify(
              "success",
              "Activado",
              "Empleado " + this.form.name + " activado",
              { duration: 3000, permanent: false }
            );
            me.$refs.vuetable.refresh();
          })
